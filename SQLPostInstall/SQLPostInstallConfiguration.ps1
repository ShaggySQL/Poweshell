...
#    $Server = $ServerName.Trim();
    ...
    try {$a = Invoke-Command -computername $Server -scriptblock {$env:computername} -ErrorAction SilentlyContinue
            if ($?)
            {   
     ...
     foreach($ins in $InsNamesdiscovery)
     {
        ...
        {
            $tport = invoke-command -computername $Server -ScriptBlock {Get-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\SuperSocketNetLib\tcp\IPAll" | select -ExpandProperty TcpPort } 
            $dport = invoke-command -computername $Server -ScriptBlock {Get-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\SuperSocketNetLib\tcp\IPAll" | select -ExpandProperty TcpDynamicPorts } 
            if($tport.Count -gt 0){ $portnumbersdiscovery += $tport}else{$portnumbersdiscovery += $dport}
     ...
     if($pnum.Count -eq 0){$pnum =$FixedPort + 1}
                 ...
                 foreach($ins in $InsNames)
                 {
                                ...
                                $tcpdynamicport = invoke-command -computername $Server -ScriptBlock {Get-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\SuperSocketNetLib\tcp\IPAll" | select -ExpandProperty TcpDynamicPorts } 
                        ...
                        {
                            invoke-command -computername $Server -ScriptBlock {Set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\SuperSocketNetLib\tcp\IPAll" -Name TcpPort -Value "$using:pnum" }
                            invoke-command -computername $Server -ScriptBlock {Set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\SuperSocketNetLib\tcp\IPAll" -Name TcpDynamicPorts -Value "" }
                            ...
                            $tcp = $wmiinstance.ServerProtocols | Where-Object { $_.DisplayName -eq 'TCP/IP' }
                                    ...
                                    echo "`nTCP/IP Protocol Enabled and port set to $pnum. Restarting the SQL service."
                                    Get-Service -Name $ins2 -ComputerName $Server | Restart-Service -Force
                               
                                
                                echo "`nSetting the agent service to Automatic and restarting the agent service."
                                ...
                                if ( $browserFlag -eq 1 )
                                {
                                    echo "`nSetting the Browser service to Automatic and restarting the browser service as its a named instance."
                                ...
                                {
                                    echo "`nSetting the Browser service to Disabled and stopping the browser service, if running, as its a default instance."
                                ...
                                {
                                echo "`nMoving the Database Files and Transaction Log files for Master, Model and MSDB databases to the default user db directories : `n$targetdatapath`n$targetlogpath "
                                ...
                                invoke-command -computername $Server -ScriptBlock {Set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\Parameters" -Name SQLArg0 -Value ("-d" + $using:masterData)}
                                invoke-command -computername $Server -ScriptBlock {Set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\Parameters" -Name SQLArg2 -Value ("-l" + $using:masterTlog)}
                                echo "`nStopping and starting SQL services."
                                Get-Service -Name $ins2 -ComputerName $Server | Stop-Service -Force
                                $('model','MSDB','mast')|ForEach-Object {invoke-command -computername $Server -ScriptBlock {Move-Item -Path $("$using:sourcelogpath" + "\" + $using:PSItem+'*.ldf') -Destination ("$using:targetlogpath" + "\") }}
                                $('model','MSDB','mast')|ForEach-Object {invoke-command -computername $Server -ScriptBlock {Move-Item -Path $("$using:sourcedatapath" + "\" + $using:PSItem+'*.mdf') -Destination ("$using:targetdatapath" + "\") }}
                                ...
                                elseif($RAM -gt 8 -and $RAM -le 12) { $min_os_memory = 4 }
                                else{ $min_os_memory = 4 }
                                ...
                                $s = New-Object -TypeName Microsoft.SqlServer.Management.Smo.Server $iName
                                echo "`nSetting Max memory settings."
                                ...
                                $num_of_cpu = invoke-command -computername $Server -ScriptBlock {(Get-WmiObject –class Win32_processor | Select -ExpandProperty DeviceID | Measure-Object).Count}
                                if ($num_of_cpu -le 8) { $maxdopvalue = $num_of_cpu/2}else{$maxdopvalue = 8}
                                echo "`nConfiguring the MaxDop Setting to : $maxdopvalue as the number of CPU's is : $num_of_cpu"
                                $s.Configuration.MaxDegreeOfParallelism.ConfigValue = $maxdopvalue
                                $s.Configuration.Alter()
                                echo "`nConfiguring the Cost Threshold For Parallelism to 50"
                                $s.Configuration.CostThresholdForParallelism.ConfigValue = 50
                                $s.Configuration.Alter()
                                echo "`nConfiguring the Initial Size to 256MB and Growth to 128MB for Model database DATA File."
                                ...
                                echo "`nConfiguring the Initial Size to 256MB and Growth to 128MB for Model database LOG File."
                                ...
                                echo "`nMaking sure 'Ole Automation Procedures' is Enabled."
                                $s.Configuration.OleAutomationProceduresEnabled.ConfigValue = 1
                                $s.Configuration.Alter()
                                echo "`nMaking sure 'Ole Automation Procedures' is Enabled."
                                $s.Configuration.OleAutomationProceduresEnabled.ConfigValue = 1
                                $s.Configuration.Alter()
                                echo "`nMaking sure 'DAC(remote admin connections)' is Disabled."
                                $s.Configuration.RemoteDacConnectionsEnabled.ConfigValue = 0
                                $s.Configuration.Alter()
                                echo "`nMaking sure 'AdHoc Distributed Queries' is Disabled."
                                $s.Configuration.AdHocDistributedQueriesEnabled.ConfigValue = 0
                                $s.Configuration.Alter()
                                echo "`nMaking sure 'IsSqlClrEnabled' is Disabled."
                                $s.Configuration.IsSqlClrEnabled.ConfigValue = 0
                                $s.Configuration.Alter()
                                echo "`nMaking sure 'CrossDBOwnershipChaining' is Disabled."
                                $s.Configuration.CrossDBOwnershipChaining.ConfigValue = 0
                                $s.Configuration.Alter()
                                echo "`nMaking sure 'ScanForStartupProcedures' is Disabled."
                                $s.Configuration.ScanForStartupProcedures.ConfigValue = 0
                                $s.Configuration.Alter()
                                echo "`nMaking sure 'XPCmdShell' is Disabled."
                                $s.Configuration.XPCmdShellEnabled.ConfigValue = 0
                                $s.Configuration.Alter()
                                echo "`nRenaming 'sa' account to 'WP_sa' and disabling it."
                                ...
                                $s.Logins.Alter()
                                echo "`nEnabling 'HideInstance' Property"
                                invoke-command -computername $Server -ScriptBlock {Set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer\SuperSocketNetLib" -Name HideInstance -Value 1}
                                echo "`nSetting number of Error logs to 12."
                                #Get-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$Using:iName\MSSQLServer" | select NumErrorLogs
                                
                                $a = invoke-command -computername $Server -ScriptBlock {(Get-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer" | select -ExpandProperty NumErrorLogs -ErrorAction silentlycontinue )}
                                if($a -gt 0)
                                {
                                    $b = invoke-command -computername $Server -ScriptBlock {Set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer" -Name NumErrorLogs -Value 12}
                                }
                                else
                                {
                                    $b = invoke-command -computername $Server -ScriptBlock {New-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\MSSQL*.$using:ins\MSSQLServer" -Name NumErrorLogs -Value 12}
                                }
                                $dbExists = $FALSE
                                ...
                                echo "`nCreating 'DBA_Admin' database."
                                $db = New-Object Microsoft.SqlServer.Management.Smo.Database -argumentlist $s, "DBA_Admin"
                                $db.Create();
                                }
                                else
                                {
                                    echo "`nDatabase 'DBA_Admin' already exists."
                                }
                                echo "`nSetting up Database Mail"
                                ...
                                echo "`nCreating operator in SQL Server Agent."
                                ...
                                echo "`nEnabling DB Mail in SQL Server Agent Alert and restarting the Agent service."
...
"@
                                if($auditcheck -eq 0)
                                {
                                    echo "`nCreating the Server Audit Specification : 'TrackLoginActivity'"
                                    $a = Invoke-SqlCmd -ServerInstance $iName -Query $auditquery
                                }
                                echo "`nCreating the OLA Maintenance Jobs and configuring the default schedule."
...
USE [dba_admin] -- Specify the database in which the objects will be created.
...
BEGIN
  ...
  SET @JobName05 = 'Admin- IntegrityCheck- SYSTEM_DATABASES'
  ...
  SET @JobName06 = 'Admin- IntegrityCheck- USER_DATABASES'
  ...
  SET @JobName07 = 'Admin- IndexOptimize- USER_DB'
  ...
  SET @JobName11 = 'Admin- CommandLog Cleanup'
...
USE [msdb]
GO
EXEC msdb.dbo.sp_update_job @job_name=N'Admin- CommandLog Cleanup', 
		@notify_level_email=2, 
		@notify_level_page=2,
		@notify_email_operator_name=N'SQL Server DBAs'
GO
USE [msdb]
GO
DECLARE @schedule_id int
EXEC msdb.dbo.sp_add_jobschedule @job_name=N'Admin- CommandLog Cleanup', @name=N'Occurs every week on Sunday at 12:00:00 AM', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
USE [msdb]
GO
EXEC msdb.dbo.sp_update_job @job_name=N'Admin- Output File Cleanup', 
		@notify_level_email=2, 
		@notify_level_page=2,
		@notify_email_operator_name=N'SQL Server DBAs'
GO
...
GO
DECLARE @schedule_id int
EXEC msdb.dbo.sp_add_jobschedule @job_name=N'Admin- Output File Cleanup', @name=N'Occurs every week on Sunday at 12:00:00 AM', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
USE [msdb]
GO
EXEC msdb.dbo.sp_update_job @job_name=N'Admin- IndexOptimize- USER_DB', 
		@notify_level_email=2, 
		@notify_level_page=2,
		@notify_email_operator_name=N'SQL Server DBAs'
GO
...
GO
DECLARE @schedule_id int
EXEC msdb.dbo.sp_add_jobschedule @job_name=N'Admin- IndexOptimize- USER_DB', @name=N'Occurs every week on Sunday at 12:00:00 AM', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
USE [msdb]
GO
EXEC msdb.dbo.sp_update_job @job_name=N'Admin- IntegrityCheck- SYSTEM_DATABASES', 
		@notify_level_email=2, 
		@notify_level_page=2, 
		@notify_email_operator_name=N'SQL Server DBAs'
GO
...
GO
DECLARE @schedule_id int
EXEC msdb.dbo.sp_add_jobschedule @job_name=N'Admin- IntegrityCheck- SYSTEM_DATABASES', @name=N'Occurs every week on Sunday at 11:00:00 PM', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
USE [msdb]
GO
EXEC msdb.dbo.sp_update_job @job_name=N'Admin- IntegrityCheck- USER_DATABASES', 
		@notify_level_email=2, 
		@notify_level_page=2, 
		@notify_email_operator_name=N'SQL Server DBAs'
GO
...
GO
DECLARE @schedule_id int
EXEC msdb.dbo.sp_add_jobschedule @job_name=N'Admin- IntegrityCheck- USER_DATABASES', @name=N'Occurs every week on Sunday at 11:30:00 PM', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
#$ServerName = "NWFLOTDBLAB1-1\TEST"
if ($ServerName -eq "" )
{
    
    echo "`nArgument missing. Usage :: ./SQLPostInstall.ps1 SERVERNAME"
    echo "Argument missing. Usage :: ./SQLPostInstall.ps1 SQLInstanceNAME   <-- Do Not provide PortNumber"
    exit;
}
else
{
    
    Perform-PostInstall $ServerName
