...
BEGIN
  ...
  INSERT INTO @Jobs ([Name], CommandTSQL, DatabaseName, OutputFileNamePart01)
  SELECT 'Admin- IndexOptimize- USER_DB',
  ...
  SELECT 'sp_purge_jobhistory',
		 ...
		 */
  INSERT INTO @Jobs ([Name], CommandTSQL, DatabaseName, OutputFileNamePart01)
  SELECT 'Admin- CommandLog Cleanup',
  ...
  SELECT 'Admin- Output File Cleanup',
  ...
  BEGIN
   UPDATE @Jobs
   SET Selected = 1
   WHERE [Name] IN('Admin- IntegrityCheck- USER_DATABASES','Admin- IndexOptimize- USER_DB','Admin- CommandLog Cleanup')
  END
  ELSE IF SERVERPROPERTY('EngineEdition') = 8
  BEGIN
   UPDATE @Jobs
   SET Selected = 1
   WHERE [Name] IN('Admin- IntegrityCheck- SYSTEM_DATABASES','Admin- IntegrityCheck- USER_DATABASES','Admin- IndexOptimize- USER_DB','Admin- CommandLog Cleanup','sp_delete_backuphistory','sp_purge_jobhistory')
  END
...
GO
  ...
  GO
		IF NOT EXISTS (
			SELECT NAME from sysjobs sj
			inner join sysjobschedules ss ON
			sj.job_id = ss.job_id
			where name = 'Admin- CommandLog Cleanup'
		)
		BEGIN
		...
		IF NOT EXISTS (
			...
			inner join sysjobschedules ss ON
			sj.job_id = ss.job_id
			where name = 'Admin- Output File Cleanup'
		)
		BEGIN
        DECLARE @schedule_id int
		...
		IF NOT EXISTS (
			...
			inner join sysjobschedules ss ON
			sj.job_id = ss.job_id
			where name = 'Admin- IndexOptimize- USER_DB'
		)
		BEGIN
        DECLARE @schedule_id int
		...
		IF NOT EXISTS (
			...
			inner join sysjobschedules ss ON
			sj.job_id = ss.job_id
			where name = 'Admin- IntegrityCheck- SYSTEM_DATABASES'
		)
		BEGIN
        DECLARE @schedule_id int
		...
		IF NOT EXISTS (
			...
			inner join sysjobschedules ss ON
			sj.job_id = ss.job_id
			where name = 'Admin- IntegrityCheck- USER_DATABASES'
		)
		BEGIN
        DECLARE @schedule_id int
