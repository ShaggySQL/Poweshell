...
$sourcebackuppath = $s.BackupDirectory
$sourceactualbackuppath = $sourcebackuppath
$sourcebackuppath = $sourcebackuppath.Replace(":","$")
$t = New-Object -TypeName Microsoft.SqlServer.Management.Smo.Server $PRODSERVER2
$targetbackuppath = $t.BackupDirectory
$targetbackuppath = $targetbackuppath.Replace(":","$")
...
$textbox7.text += "3. Creating a test database : TestAGdb `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
$db = New-Object Microsoft.SqlServer.Management.Smo.Database -argumentlist $s, "TestAGdb"
$db.Create();
}
else
{
    $textbox7.text += "3. Test database : TestAGdb already exists`r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
}
if (Test-Path "\\$PRD1\$sourcebackuppath\TestAGdb.bak") 
{
  Remove-Item "\\$PRD1\$sourcebackuppath\TestAGdb.bak"
}
if (Test-Path "\\$PRD1\$sourcebackuppath\TestAGdb.trn") 
{
  Remove-Item "\\$PRD1\$sourcebackuppath\TestAGdb.trn"
}
if (Test-Path "\\$PRD2\$targetbackuppath\TestAGdb.bak") 
{
  Remove-Item "\\$PRD2\$targetbackuppath\TestAGdb.bak"
}
if (Test-Path "\\$PRD2\$targetbackuppath\TestAGdb.trn") 
{
  Remove-Item "\\$PRD2\$targetbackuppath\TestAGdb.trn"
}
$textbox7.text += "4. Executing TestAGdb database backup `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
try{
$a= Backup-SqlDatabase -Database "TestAGdb" -BackupFile "$sourceactualbackuppath\TestAGdb.bak" -ServerInstance "$PRODSERVER1"  
}
catch
{
$backupError = "Backup Database Failed with the below error:`r`n`r`nError Message:`r`n"
$backupError = $backupError + $error[0].Exception
$textbox7.text += "Backup Database Failed. `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
[System.Windows.MessageBox]::Show($backupError)

$Okbutton.Enabled = $true; Return;
}
$a= Backup-SqlDatabase -Database "TestAGdb" -BackupFile "\\$PRD1\$sourcebackuppath\TestAGdb.trn" -ServerInstance "$PRODSERVER1" -BackupAction Log  
$textbox7.text += "5. Copying TestAGdb database backup files `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
$a= Copy-Item "\\$PRD1\$sourcebackuppath\TestAGdb.bak" -destination "\\$PRD2\$targetbackuppath"
$a= Copy-Item "\\$PRD1\$sourcebackuppath\TestAGdb.trn" -destination "\\$PRD2\$targetbackuppath"
$textbox7.text += "6. Restoring TestAGdb database `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
$a= Restore-SqlDatabase -Database "TestAGdb" -BackupFile "\\$PRD2\$targetbackuppath\TestAGdb.bak" -ServerInstance "$PRODSERVER2" -ReplaceDatabase -NoRecovery
$a= Restore-SqlDatabase -Database "TestAGdb" -BackupFile "\\$PRD2\$targetbackuppath\TestAGdb.trn" -ServerInstance "$PRODSERVER2" -RestoreAction Log -NoRecovery

$textbox7.text += "7. Creating and starting the Endpoints on both the servers, if it isn't already. `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
$textbox7.text += "8. Granting CONNECT ON ENDPOINT permissions and Starting the extended events session for AlwaysOn. `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
        $s = New-Object -TypeName Microsoft.SqlServer.Management.Smo.Server $PRODSERVER1
...
$textbox7.text += "9. Creating the Availability group $ag1 `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
...
#$textbox7.text +=$test
$a = New-SqlAvailabilityGroup -InputObject "$PRD1,$port" -Name $ag1 -AvailabilityReplica @($ag1primaryReplica,$ag1secondaryReplica) -AutomatedBackupPreference Primary -Database "TestAGdb" 
}
catch
{
...
$textbox7.text += "10. Joining the secondary replica $PRD2 to the Availability group $ag1 `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
$a = Join-SqlAvailabilityGroup -Path "SQLSERVER:\SQL\$PRD2\$aginstance,$port" -Name $ag1  
$textbox7.text += "11. Joining the secondary database TestAGdb to the Availability group $ag1 `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();
$a = Add-SqlAvailabilityDatabase -Path "SQLSERVER:\SQL\$PRD2\$aginstance,$port\AvailabilityGroups\$ag1" -Database "TestAGdb" 
$textbox7.text += "12. Creating the listener. `r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret(); 
...
$textbox7.text += "13. Listener created successfully. `r`n14. Availability Group created successfully`r`n";$textbox7.SelectionStart = $textbox7.TextLength;$textbox7.ScrollToCaret();[System.Windows.Forms.Application]::DoEvents(); 
...
$Form.Text = "AG Creation"

$Label1 = New-Object System.Windows.Forms.Label
$Label1.Text = "Please enter the below details to create an AvailabilityGroup"
