...
try
{
    ...
    foreach ($ag in $AGDetails)
    {
        ...
        $AGPrimaryReplicaAvailabilityMode = $ag.AvailabilityReplicas[$AGPrimaryReplicaName].AvailabilityMode
        ...
        $jobdetails = $sqlserver.JobServer.Jobs | where {$_.Name -eq "test"}|select name,currentrunstatus
            ...
            foreach ($jb in $jobdetails)
            {
                ...
                else
                {
                   ...
                   echo "`n`n`n$jobname job Completed."
                     ...
                     $jobdetails = $sqlserver.JobServer.Jobs | where {$_.Name -eq "test2" }|select name,currentrunstatus
                    foreach ($jb in $jobdetails)
                    {
                        $jobname = $jb.name
                        if($jb.CurrentRunStatus -ne "Idle")
        ...
        {
                    $sqlserver = New-Object -TypeName Microsoft.SqlServer.Management.Smo.Server $hostsqlserver;
                    $AGRemotesecondaryReplicaAvailabilityMode = $sqlserver.AvailabilityGroups[$ag.Name].AvailabilityReplicas[$AGRemotesecondaryReplicaName].AvailabilityMode
                    $AGPrimaryReplicaAvailabilityMode = $sqlserver.AvailabilityGroups[$ag.Name].AvailabilityReplicas[$AGPrimaryReplicaName].AvailabilityMode
                    ...
                    echo "`n`n`nSuccessfully failed over. ";
                    echo "`n`n`n$AGRemotesecondaryReplicaName Availability Mode is : $AGRemotesecondaryReplicaAvailabilityMode "
                    echo "`n`n`n$AGprimaryReplicaName Availability Mode is : $AGPrimaryReplicaAvailabilityMode "
